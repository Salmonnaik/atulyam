...
export default function EventCard({
  ...
  return (
    ...
    >
      ...
      {/* Left glow bar */}
      <div
        className="absolute left-0 top-0 bottom-0 w-1 opacity-0 group-hover:opacity-100 transition-opacity duration-400"
        style={{
      ...
      <div
        ...
        style={{
          background: `radial-gradient(circle at 50% 0%,${event.color}15,transparent 65%)`,
        }}
      />
      ...
      <div className="p-8 md:p-10 relative z-10">
        ...
        <div className="flex items-start justify-between mb-5">
          <span className="text-5xl filter drop-shadow-[0_0_10px_rgba(255,255,255,0.2)]">
            {event.icon}
          </span>
          <div className="text-right">
            <div
              className="font-mono text-[0.52rem] tracking-[2px] uppercase mb-1"
        ...
        <h3
          className="font-cinzel text-xl tracking-[2px] mb-1"
          style={{ color: theme.text }}
        >
          {event.name}
        </h3>
        <p
          className="font-jp text-[0.78rem] tracking-[3px] mb-5"
          style={{ color: event.color }}
        >
          {event.tagline}
        </p>
        <p
          className="text-[0.95rem] leading-[1.75] font-light mb-6"
        ...
        <div className="flex flex-wrap gap-2 mb-6">
          {event.tags.map((t) => (
            <span
              key={t}
              className="px-3 py-1 font-mono text-[0.56rem] tracking-[2px] uppercase border"
        ...
        <div
          className="grid grid-cols-2 gap-3 p-4 mb-6 border"
          ...
          {[
            ...
            <div key={l}>
              ...
              <div
                className="text-sm"
                style={{
                  color: l.includes("Prize") ? "#ffd700" : theme.textMid,
                }}
              >
                {v}
              </div>
        ...
        {event.rules?.length || event.guidelines?.length ? (
          <div className="space-y-3 mb-6">
            {event.rules?.length ? (
              ...
              >
                <summary
                  className="font-mono text-[0.62rem] tracking-[2px] uppercase cursor-pointer"
                  style={{ color: theme.text }}
                >
                  Rules ({event.rules.length})
                </summary>
                <ul
                  className="mt-3 space-y-2 text-[0.82rem] list-disc pl-5"
                  style={{ color: theme.textMid }}
                >
            ...
            {event.guidelines?.length ? (
              ...
              >
                <summary
                  ...
                  style={{ color: theme.text }}
                >
                  Guidelines ({event.guidelines.length})
                </summary>
                <ul
                  className="mt-3 space-y-2 text-[0.82rem] list-disc pl-5"
                  style={{ color: theme.textMid }}
        ...
        <motion.a
          ...
          style={{
            ...
            boxShadow: hov ? `0 0 28px ${event.color}55` : "none",
          }}
        >
          Register Now ✦
        </motion.a>
